...
class Dummy02Module(BaseModule):
    ...
    async def initialize(self):
        ...
        # 注册API路由
        await self.add_route("/transform", module_category="agent",
                       methods=["POST"], handler=self.handle_transform)

        # 订阅事件
        await self.subscribe("dummy02.transform", self.handle_transform_event)
        await self.subscribe("dummy02.process", self.handle_process_event)
