...
class ScheduledPlaylistConfig(ModuleConfig):
    playlist: str = field(default="", metadata={
        "required": True,
        "desc": "定时播放列表，按JSON格式填写：{\"【歌曲名字】\":{\"file_name\":\"【歌曲文件路径】\",\"subtitle\":\"【歌曲字幕路径】\",\"vocal\":\"【歌曲纯人声音频路径】\",\"start_time\":【歌曲开始播放时间戳】}}，注意只支持 wav 格式的歌曲和 srt 格式的字幕",
        "multiline": True
    })
    ...
    async def process_task(self, task: Message[Any] | None) -> Message[Any] | None:
        for song_id, song_info in self.playlist.items():
            if time() >= song_info["start_time"]:
                ...
                return SongInfo(self, song_id, song_info["file_name"], song_info["vocal"], song_info["subtitle"])
