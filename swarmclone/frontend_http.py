...
class FrontendHTTP(ModuleBase):
    def __init__(self):
        super().__init__(ModuleRoles.FRONTEND, "FrontendHTTP")
    ...
    async def run(self):
        ...
        while True:
            ...
            for address, url in self.urldict.items():
                loop.create_task(self.process_task_with_url(url, task))
        
    async def process_task_with_url(self, url: str, task: Message):
        if await self.is_url_accessible(url):
            ...
            async with (aiohttp.ClientSession() as session,
                    session.post(url, headers=headers, json=message["json"], data=message["data"]) as response):
    ...
    async def is_url_accessible(self, url: str) -> bool:
        try:
            async with (aiohttp.ClientSession() as session,
                    session.head(url, allow_redirects=True, timeout=5.0) as response):
