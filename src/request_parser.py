...
def loads(request_str: str) -> list[RequestType]:
    request_strings = request_str.split(str(global_config.server.requests_separator))
    requests = []
...
def dumps(requests: list[RequestType]) -> str:
    return "".join([
        (json.dumps(request).replace(str(global_config.server.requests_separator), "") + # 防止在不应出现的地方出现分隔符
         str(global_config.server.requests_separator))
        for request in requests
    ])

class Loader: # loads的进一步封装
    def __init__(self, global_config):
        self.sep = global_config.server.requests_separator
        self.request_str = ""
